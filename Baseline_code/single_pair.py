...
'''
该代码用于ElasticSearch搜索结果形成 按文章共显统计 pair 文件 / 单个药物所有 pair 对统计文件 / 所有 nodes 统计文件*2
 *_spair 是以文章分割的每个文章内共显的Gene-Chemical pair
 *_all_pair 是一个疾病所有检索结果的 Gene-Chemical pair
 *_nodes 是疾病检索结果中所有pubtator注释结果（Gene， Chemical）在pair中出现的次数统计，及nodes的度
 
 1.24 
 添加根据Homo_sapiens.gene_info 添加 gene id 转为 symbol功能
 并且过滤掉pubtator 注释的 非智人 的 gene
'''
...
def Result_pair(rf, out, node_a, node_b, index_file, mesh, ebi, supp):
    ...
    single_pair.write('# aa means {0}-{0}, bb means {1}-{1}, ab means {0}-{1}\n'.format(node_a, node_b))
