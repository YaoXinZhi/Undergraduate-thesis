...
def co_sentence(json_file, outfile):
    ...
    SENTENCE_COUNT=0
    SENT_COUNT = open('SENT_COUNT_2', 'a')
    ...
    with open(json_file) as f:
        for line in f:
            ...
            # text 分句 并计算sentence 的 offset 信息
            ...
            if result_list != []:
                ...
                for sent in result_list:
                    ...
                    rfile.write('\n')
                rfile.write('\n')
    SENT_COUNT.write(SENTENCE_COUNT)

    rfile.close()
    SENT_COUNT.close()
    time_end = time.time()
    print ('totally step: {0} totally cost: {1}'.format(STEP, time_end-time_start))
